...
namespace Microsoft.StreamProcessing
{
    internal sealed class FusedObservable<TKey, TInput, TPayload, TResult, TOutput> : IObservable<TOutput>
    {
        ...
        public IDisposable Subscribe(IObserver<TOutput> observer)
        {
            ...
            Expression<Action<TInput>> onNextFinal;
            if (this.fuseModule.HasDurationAdjustment)
            {
                // Use a mutable local for the end time so that Assign(otherParam, ...) generated
                // by FuseModule.Coalesce for duration adjustments compiles correctly even when
                // the end-edge extractor targets a readonly field (e.g. StreamEvent<T>.OtherTime).
                var otherLocal = Expression.Variable(typeof(long), "_other");
                ...
                onNextFinal = Expression.Lambda<Action<TInput>>(
                    Expression.Block(
                        [otherLocal],
                        Expression.Assign(otherLocal, this.endEdgeExtractor.Body),
                        onNextReplacedParameters
                    ),
                    this.inputParameter
                );
            }
            ...
            {
                // No duration adjustment — no Assign(otherParam) is generated by Coalesce, so we
                // can substitute the end-edge extractor body directly without a mutable local.
                var onNextReplacedParameters = ParameterSubstituter.Replace(
                    onNextFused.Parameters,
                    onNextPunctuation,
                    this.startEdgeExtractor.Body,
                ...
                onNextFinal = Expression.Lambda<Action<TInput>>(onNextReplacedParameters, this.inputParameter);
            }
