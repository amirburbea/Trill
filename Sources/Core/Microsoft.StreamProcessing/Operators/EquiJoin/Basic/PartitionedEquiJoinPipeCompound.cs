...
namespace Microsoft.StreamProcessing
{
    ...
    internal sealed class PartitionedEquiJoinPipeCompound<TGroupKey, TLeft, TRight, TResult, TPartitionKey> : BinaryPipe<CompoundGroupKey<PartitionKey<TPartitionKey>, TGroupKey>, TLeft, TRight, TResult>, IBinaryObserver
    {
        ...
        private void ProcessLeftEvent(PartitionEntry partition, long start, long end, ref TGroupKey key, TLeft payload, int hash)
        {
            if (start < end)
            {
                ...
                if (end == StreamEvent.InfinitySyncTime)
                {
                    ...
                    if (processable)
                    {
                        ...
                        this.CreateOutputForStartEdge(partition, start, ref key, ref payload, hash);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (processable)
                    {
                        ...
                        this.CreateOutputForStartInterval(partition, start, end, ref key, ref payload, hash);
            ...
            else
            {
                ...
                if (!partition.isRightComplete)
                {
                    ...
                    {
                        if (this.AreSame(end, ref key, ref payload, ref partition.leftEdgeMap.Values[index]))
                        {
                            edges.Remove();
                            break;
                        }
                ...
                // Output end edges.
                this.CreateOutputForEndEdge(partition, start, end, ref key, ref payload, hash);
            }
        }
        ...
        private void ProcessRightEvent(PartitionEntry partition, long start, long end, ref TGroupKey key, TRight payload, int hash)
        {
            ...
            else
            {
                ...
                if (!partition.isLeftComplete)
                {
                    ...
                    while (edges.Next(out int index))
                    {
                        if (this.AreSame(end, ref key, ref payload, ref partition.rightEdgeMap.Values[index]))
                        {
                            edges.Remove();
                            break;
                        }
        ...
        private struct ActiveInterval<TPayload>
        {
            ...
            public override readonly string ToString() => "[Start=" + this.Start + ", End=" + this.End + ", Key='" + this.Key + "', Payload='" + this.Payload + "']";
        }
        ...
        private sealed class PartitionEntry
        {
            ...
            /// </summary>
            [DataMember]
            public FastMap<ActiveEdge<TLeft>> leftEdgeMap = new();
            ...
            /// </summary>
            [DataMember]
            public FastMap<ActiveEdge<TRight>> rightEdgeMap = new();
