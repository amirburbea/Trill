...
namespace Microsoft.StreamProcessing.Internal
{
    internal static class FastDictionaryGenerator
    {
        ...
        {
            if (EqualityComparerExpression<TKey>.IsSimpleDefault(comparerExp)) return static () => new();
            ...
            if (vars.Count == 0) vars.Add(string.Empty.StableHash());
            var hashvars = VariableFinder.Find(getHashCodeExp).Select(o => o.GetHashCode()).ToList();
            if (hashvars.Count == 0) hashvars.Add(string.Empty.StableHash());
            var key =
                Tuple.Create(
                    equalsExp.ToString() + getHashCodeExp.ToString() + string.Concat(vars.Aggregate(static (a, i) => a ^ i)) + string.Concat(hashvars.Aggregate(static (a, i) => a ^ i)),
    ...
    internal partial class GeneratedFastDictionary(string name, string dictType)
    {
    }
}
